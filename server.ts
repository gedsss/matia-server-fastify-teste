import Fastify, { FastifyInstance, FastifyRegisterOptions, FastifyReply, FastifyRequest } from 'fastify';
...
// --- REGISTRO DE ROTAS ---
await fastify.register(profileRoutes, { prefix: '/api/profile' } as FastifyRegisterOptions<any>) ;
await fastify.register(userRoleRoutes, { prefix: '/api/user_roles' } as FastifyRegisterOptions<any>);
await fastify.register(messagesRoutes, { prefix: '/api/messages' } as FastifyRegisterOptions<any>);
await fastify.register(documentsRoutes, { prefix: '/api/documents' } as FastifyRegisterOptions<any>);
await fastify.register(documentsTagsRoutes, { prefix: '/api/documents_tags' } as FastifyRegisterOptions<any>);
await fastify.register(documentsTagsRelationsRoutes, { prefix: '/api/documents_tags_relations' } as FastifyRegisterOptions<any>);
await fastify.register(documentsAnalysesRoutes, { prefix: '/api/documents_analyses' } as FastifyRegisterOptions<any>);
await fastify.register(conversationsRoutes, { prefix: '/api/conversations' } as FastifyRegisterOptions<any>);
await fastify.register(conversationDocumentsRoutes, { prefix: '/api/conversation_documents' } as FastifyRegisterOptions<any>);
await fastify.register(activityLogsRoutes, { prefix: '/api/activity_logs' } as FastifyRegisterOptions<any>);
await fastify.register(userActivityLogsRoutes, { prefix: '/api/user_activty_log' } as FastifyRegisterOptions<any>);
...
fastify.setErrorHandler((error, request, reply) => {
  ...
  if (error && error.name === 'ZodError') {
    return reply.code(400).send({
      ...
      issues: (error as any).errors // Assumindo a estrutura do Zod
    })
  }
  */
  reply.send(error);
