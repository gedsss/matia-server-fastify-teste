...
export const createDocumentsTagsRelation = async (request, reply) => {
  try {
    ...
    return success(reply, 201, { data: created, message: 'relação de tag criado com sucesso' })
  } catch (err) {
...
export const updateDocumentsTagsRelation = async (request, reply) => {
  try {
    ...
    return success(reply, 200, { data: updated, message: 'relação de tag atualizado' })
  } catch (err) {
    if (err && err.name === 'SequelizeValidationError') {
      return fail(reply, 400, 'Dados inválidos', err.errors)
    }
